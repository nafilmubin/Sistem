...
<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        if($number > 1)
        {
            for($i=0; $i<$number; $i++)
            {
                mysqli_query($koneksi, "UPDATE rab_detail SET nama_anggaran='".$_POST["nama_anggaran"][$i]."', nominal='".$_POST["nominal"][$i]."', status='".$_POST["status"][$i]."' WHERE id_det='".$_POST["id_det"][$i]."'");
        ...
        $result = mysqli_query($koneksi, "UPDATE rab SET keterangan='Telah Ditanggapi' WHERE id_rab='".$_POST['id_rab']."'");
